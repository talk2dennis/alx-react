...
describe("when isLogged in is true", () => {
  ...
  it(`Tests that the logIn function updates user's state correctly`, () => {
    const wrapper = mount(
      <AppContext.Provider value={{ defaultUser, defaultLogOut }}>
        <App />
      </AppContext.Provider>
    );
...
describe('mapStateToProps', () => {
  it('should return the correct object when isUserLoggedIn is true', () => {
    // Create a state with isUserLoggedIn set to true
    let state = fromJS({
      isUserLoggedIn: true,
    });

    // Call mapStateToProps with this state
  ...
  it('should return the correct object when isUserLoggedIn is false', () => {
    // Create a state with isUserLoggedIn set to false
    let state = fromJS({
